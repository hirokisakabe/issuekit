...
| --- | --- |
| Codex CLI | A single `issue-implement` request on the default branch hands the issue to `issue-dispatch`. The parent starts one non-interactive worker with [`codex exec --approve-for-me --worktree`](https://developers.openai.com/codex/cli/reference), without migrating its own cwd. Auto-review provides `workspace-write`; Codex owns the managed worktree path and lifecycle; the worker verifies isolation and establishes its expected issue branch before editing. |
