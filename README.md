...
issuekit ships the following Claude Code skills under `skills/`:
...
This installs all six skills under your agent's skill directory (e.g. `~/.claude/skills/` for Claude Code). To install a specific skill only:

```bash
npx skills add hirokisakabe/issuekit --skill issue-implement
```

See [skills.sh](https://skills.sh) for the list of supported agents (Claude Code, Codex CLI, Cursor, Gemini, ...).
