...
### 6. cross-review (issuekit:cross-review skill)
...
cross-review の指摘を反映した修正後に、commit 前で受け入れ条件が崩れていないかを最終確認するステップ。Step 4 の実装中にも `acceptance-check` 相当の検証を行うが、cross-review 由来の修正で受け入れ条件を踏み外していないかをここで改めて検査する位置づけ。
