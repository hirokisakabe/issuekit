...
description: GitHub issue 番号を起点に、Status 確認 → worktree 化 → 実装 → cross-review → 受け入れ条件チェック → commit → PR 作成 → CI 確認まで一気通貫で進める。ユーザーから「issue #N を実装して」「#N お願い」などの依頼があった場合に使用する。
---
...
- **`issuekit:cross-review` skill**: 実装後の cross-review に必須。APM plain-skill mode では `cross-review` として呼び出す。backend は環境変数 `CROSS_REVIEW_BACKEND` で `codex` / `claude-self` から選択でき、未指定時は利用可能な CLI を自動検出する。対応する CLI がいずれも未導入な場合は明確に失敗させる（該当 skill 側の失敗時対応に従う）。
...
- **含む**: Status 確認、Depends on の close 確認、親 issue の文脈取り込み、worktree への自動切り替え (Claude Code 環境かつ default branch 上のときのみ、条件付き)、実装、cross-review、受け入れ条件チェック、commit、PR 作成、CI 確認・修正。
- **含まない**:
...
### 7. cross-review (issuekit:cross-review skill)

`issuekit:cross-review` skill を呼び出して、別 backend (Codex CLI または Claude CLI headless) による cross-review を実施する。APM plain-skill mode では `cross-review` を呼び出す。
...
### 8. 受け入れ条件チェック (issuekit:acceptance-check skill)

cross-review の指摘を反映した修正後に、commit 前で受け入れ条件が崩れていないかを最終確認するステップ。Step 5 の実装中にも `acceptance-check` 相当の検証を行うが、cross-review 由来の修正で受け入れ条件を踏み外していないかをここで改めて検査する位置づけ。
